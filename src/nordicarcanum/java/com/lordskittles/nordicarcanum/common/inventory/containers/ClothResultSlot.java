    ...
    public ItemStack onTake(@Nonnull PlayerEntity thePlayer, @Nonnull ItemStack stack)
    {
        ...
        for (int i = 0; i < inventoryItems.size(); ++i)
        {
            ...
            if (!inventoryStack.isEmpty())
            {
                ...
                else if (ItemStack.areItemsEqual(itemstack, inventoryStack) && ItemStack.areItemStackTagsEqual(itemstack, inventoryStack))
                {
                    inventoryStack.grow(itemstack.getCount());
                    this.craftMatrix.setInventorySlotContents(i, inventoryStack);
                }
                else if (!this.player.inventory.addItemStackToInventory(inventoryStack))
                {
